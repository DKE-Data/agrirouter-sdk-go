			...
			case messageSentTestEvent := <-s.sentMessagesTestEvents:
				...
				if isFileMessageType(messageSentTestEvent.MessageType) {
					...
					eventData := MessageReceivedEventData{
						// app message id is generated by agrirouter, chunk index appended
						AppMessageId:        messageId.String() + "-0",
...
func (s *Server) ListCompatibleApplications(
	_ context.Context,
	_ ListCompatibleApplicationsRequestObject,
) (ListCompatibleApplicationsResponseObject, error) {
	return ListCompatibleApplications200JSONResponse{CompatibleApplications: []CompatibleApplication{}}, nil
}
